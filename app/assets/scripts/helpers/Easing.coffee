HELPER.Easing = {
  ...
  ExponentialEaseIn: (p)->
    return if (p == 0.0) then p else Math.pow(2, 10 * (p - 1))

  # Modeled after the exponential function y = -2^(-10x) + 1
  ExponentialEaseOut: (p)->
    return if (p == 1.0) then p else 1 - Math.pow(2, -10 * p)

  # Modeled after the piecewise exponential
  # y = (1/2)2^(10(2x - 1))         ; [0,0.5)
  # y = -(1/2)*2^(-10(2x - 1))) + 1 ; [0.5,1]
  ExponentialEaseInOut: (p)->
    if(p == 0.0 || p == 1.0)
      return p
